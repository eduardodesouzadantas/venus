...
import { ShieldCheck, UserCircle2, Briefcase, Lock, Mail, Eye, EyeOff, ArrowRight } from "lucide-react";
...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Default org for merchant login
      const targetOrg = role === 'merchant' ? 'maison-elite' : undefined;
      const targetRole = role === 'merchant' ? 'merchant_owner' : 'agency_owner';

      await login(email || "admin@venus.ai", targetRole, targetOrg, password || "venus-demo-password");
    } catch (error) {
      ...
      <div className="w-full max-w-sm space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
         ...
         <div className="p-1.5 rounded-full bg-white/5 border border-white/10 flex">
            <button 
              ...
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${role === 'merchant' ? "bg-white text-black shadow-xl" : "text-white/40 hover:text-white"}`}>
               <Briefcase size={14} /> Lojista
            </button>
            <button 
              onClick={() => setRole('agency')}
              className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${role === 'agency' ? "bg-[#D4AF37] text-black shadow-[0_0_20px_rgba(212,175,55,0.4)]" : "text-white/40 hover:text-white"}`}>
         ...
         <form onSubmit={handleLogin} className="space-y-6">
            ...
            <VenusButton 
              ...
              disabled={isLoading}
              variant="solid" 
              className={`w-full py-8 h-auto text-[11px] font-bold uppercase tracking-[0.4em] rounded-full shadow-2xl active:scale-[0.98] transition-all ${role === 'agency' ? "bg-[#D4AF37] text-black" : "bg-white text-black"}`}>
               {isLoading ? "Validando Acesso..." : (role === 'merchant' ? "Entrar na Maison Elite" : "Entrar como Master")}
               <ArrowRight className="w-4 h-4 ml-2" />
            </VenusButton>

            <button type="button" className="w-full text-[10px] text-white/20 uppercase font-bold tracking-widest hover:text-white transition-colors">Esqueci minha senha</button>
         </form>
