         ...
         <header className="flex items-center justify-between">
            <div className="space-y-1">
               <div className="flex items-center gap-2">
                  ...
                  <Text className="text-[10px] uppercase font-bold tracking-[0.4em] text-[#D4AF37]">WhatsApp Marketing Executive</Text>
               </div>
            ...
            <div className="flex gap-4">
               <div className="px-6 rounded-full border border-white/5 bg-white/[0.02] flex items-center gap-4 h-12">
                  ...
                  <span className="text-[10px] font-bold text-[#D4AF37] uppercase tracking-widest">Connect: Active</span>
               </div>
               {!showBuilder && (
            ...
            <section className="grid grid-cols-5 gap-12 animate-in slide-in-from-bottom-4 duration-700">
               ...
               <div className="col-span-2 space-y-10 border-r border-white/5 pr-12 pb-20">
                  <div className="space-y-6">
                     ...
                     <div className="grid grid-cols-1 gap-4">
                        <div className="space-y-2">
                           <label className="text-[9px] uppercase font-bold tracking-widest text-[#D4AF37] ml-4 italic">Objetivo da Campanha</label>
                           <select 
                             value={campaign.objective}
                             onChange={(e) => setCampaign({ ...campaign, objective: e.target.value as CampaignObjective })}
                             className="w-full h-14 bg-white/5 border border-white/10 rounded-3xl px-6 text-xs text-white focus:border-[#D4AF37]/40 outline-none transition-all appearance-none uppercase tracking-widest font-bold">
                        ...
                        <div className="p-6 rounded-[32px] bg-[#D4AF37]/5 border border-[#D4AF37]/20 flex items-center justify-between">
                           <div className="flex items-center gap-3">
                              <Target size={16} className="text-[#D4AF37]" />
                              <span className="text-[10px] uppercase font-bold tracking-widest text-[#D4AF37]">Alcance Estimado</span>
                           </div>
                  ...
                  <div className="space-y-6">
                     ...
                     <VenusButton onClick={handleGenerateAI} variant="solid" className="w-full py-6 h-auto bg-[#D4AF37] text-black rounded-full text-[10px] font-bold uppercase tracking-[0.4em] flex items-center justify-center gap-2 group active:scale-95 transition-all">
                  ...
                  <div className="flex items-center justify-between px-2">
                     <div className="flex gap-8">
                        <button 
                          ...
                          className={`text-xs uppercase tracking-[0.4em] font-bold transition-all ${activeTab === 'active' ? "text-[#D4AF37]" : "text-white/20 hover:text-white"}`}>
                           Ativas
                        </button>
                        <button 
                          onClick={() => setActiveTab('history')}
                          className={`text-xs uppercase tracking-[0.4em] font-bold transition-all ${activeTab === 'history' ? "text-[#D4AF37]" : "text-white/20 hover:text-white"}`}>
                           Histórico
                        </button>
                     </div>
                       ...
                       <div key={c.id} className="p-8 rounded-[48px] bg-white/[0.02] border border-white/5 flex items-center justify-between group hover:bg-white/[0.04] transition-all relative overflow-hidden">
                          ...
                          <div className="flex items-center gap-10">
                             <div className="w-16 h-16 rounded-[32px] bg-white/5 border border-white/10 flex items-center justify-center relative overflow-hidden">
                                <MessageSquare size={24} className="text-[#D4AF37]/40" />
                             </div>
...
function Metric({ value, label, highlight = false }: { value: any, label: string, highlight?: boolean }) {
  return (
    <div className="flex flex-col items-end">
       ...
       <span className={`text-xl font-serif ${highlight ? "text-[#D4AF37]" : "text-white/60"}`}>{value}</span>
    </div>
  );
}
