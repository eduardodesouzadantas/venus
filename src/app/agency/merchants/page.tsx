    ...
    <main className="min-h-screen bg-[var(--bg)] p-4 text-[var(--text)] sm:p-6" style={themeVars}>
      <section className="mx-auto max-w-[1500px] space-y-5">
        <header className="flex flex-col gap-4 border border-[var(--border)] bg-[var(--bg2)] p-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">
              Controle operacional de todas as orgs com semaforos, metricas e acoes de administracao.
            </p>
          </div>
          <Link
        ...
        <section className="border border-[var(--border)] bg-[var(--border)]">
          <div className="hidden grid-cols-[1.3fr_0.8fr_0.8fr_0.8fr_0.8fr_1fr_1.8fr] gap-[1px] bg-[var(--border)] font-mono text-[9px] uppercase tracking-[0.18em] text-[var(--muted)] xl:grid">
            {["Loja", "Slug", "Plano", "Status", "Leads", "Try-ons", "Acoes"].map((label) => (
          ...
          <div className="divide-y divide-[var(--border)] bg-[var(--bg2)]">
            {rows.length > 0 ? (
              rows.map((row) => (
                <article
                  key={row.id}
                  className="grid gap-4 px-4 py-4 xl:grid-cols-[1.3fr_0.8fr_0.8fr_0.8fr_0.8fr_1fr_1.8fr] xl:items-center xl:gap-3"
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[var(--muted)]">
                          criada {formatDate(row.created_at)}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <ActionShell>
                    <Link className="border border-[var(--green)]/35 px-3 py-2 text-[var(--green)] hover:bg-[var(--green)] hover:text-black" href={`/org/${row.slug}/dashboard`}>
                      Entrar na loja
                    </Link>
                    <Link className="border border-[var(--gold)]/45 px-3 py-2 text-[var(--gold)] hover:bg-[var(--gold)] hover:text-black" href={`/agency/merchants/${row.id}`}>
                      Editar
                    </Link>
              ...
              <div className="p-8 text-center">
                ...
                <p className="mt-2 text-sm text-[var(--muted)]">Nenhuma org encontrada ou Supabase indisponivel.</p>
              </div>
            )}
          </div>
        </section>
