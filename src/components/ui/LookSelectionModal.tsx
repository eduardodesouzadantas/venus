...
export function LookSelectionModal({ look, onClose, onSelectItem, onRequestWhatsApp }: LookSelectionModalProps) {
  ...
  const hasLegacyProducts = !!look && look.items.some((item) => !item.product_id);
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center px-6 pb-20">
             ...
             <div key={item.id} className="flex items-center justify-between group p-4 rounded-3xl bg-white/[0.03] border border-white/5 active:scale-[0.98] transition-all">
                ...
                <div className="flex flex-col items-end gap-3">
                    <button
                       ...
                       onClick={() => {
                         if (!item.product_id) {
                           console.warn("[TRYON] legacy item without product_id", {
                             lookId: look.id,
                             itemId: item.id,
                           });
                           return;
        ...
        {hasLegacyProducts && (
          <div className="w-full mb-10 rounded-[32px] border border-[#D4AF37]/20 bg-[#D4AF37]/6 px-5 py-4 text-center">
            <p className="text-[10px] font-bold uppercase tracking-[0.22em] text-[#D4AF37]">
              Try-on indisponível neste legado
            </p>
            <p className="mt-2 text-xs leading-relaxed text-white/55">
              Esse resultado foi salvo antes do Venus registrar o UUID real do produto.
            </p>
          </div>
        )}
